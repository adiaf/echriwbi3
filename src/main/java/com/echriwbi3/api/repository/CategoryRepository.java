...
public interface CategoryRepository extends MongoRepository<Category, String>, QuerydslPredicateExecutor<Category> {
        ...
        Page<Category> findByParentCategoryIdAndCategoryLevelAndNameContainingAndEnabeledTrue(
                        @Param("categoryId") Set<String> categoryIds, @Param("level") Integer level,
                        @Param("name") String name, Pageable pageable);

        @Query("{ 'parentCategory.$id':{ '$in' :[{$oid : ?0}]},'categoryLevel':?1,'enabeled':true}")
        Page<Category> findByParentCategoryIdAndCategoryLevelAndEnabeledTrue(
                        @Param("categoryId") Set<String> categoryIds, @Param("level") Integer level, Pageable pageable);

        Page<Category> findByParentCategoryIdAndNameContainingAndEnabeledTrue(
                        @Param("categoryId") Set<String> categoryIds, @Param("name") String name, Pageable pageable);
}
